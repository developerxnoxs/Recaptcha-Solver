...
async function analyzeBoundingBoxWithGemini(screenshotPath, promptText, canvasSize, apiKey, enableScreenshot = false) {
    ...
    try {
        ...
        const actualWidth = canvasSize.displayWidth || canvasSize.width;
        const actualHeight = canvasSize.displayHeight || canvasSize.height;
...
🎯 TASK: "${promptText}"
📐 Image: ${actualWidth} × ${actualHeight} pixels

⚠️ CRITICAL: The coordinates you provide MUST be based on THIS image size (${actualWidth}×${actualHeight} pixels).
This is the EXACT size of the screenshot you're analyzing.
...
Mentally divide the image into a 3×3 grid:
┌─────────┬─────────┬─────────┐
│ TOP-L   │ TOP-C   │ TOP-R   │  Y: 0-${Math.floor(actualHeight/3)}
├─────────┼─────────┼─────────┤
│ MID-L   │ MID-C   │ MID-R   │  Y: ${Math.floor(actualHeight/3)}-${Math.floor(actualHeight*2/3)}
├─────────┼─────────┼─────────┤
│ BOT-L   │ BOT-C   │ BOT-R   │  Y: ${Math.floor(actualHeight*2/3)}-${actualHeight}
└─────────┴─────────┴─────────┘
X: 0-${Math.floor(actualWidth/3)}  ${Math.floor(actualWidth/3)}-${Math.floor(actualWidth*2/3)}  ${Math.floor(actualWidth*2/3)}-${actualWidth}
...
✓ Are coordinates within image bounds (0-${actualWidth}, 0-${actualHeight})?
...
Calculate with MAXIMUM PRECISION!`;
        ...
        logger.debug('Gemini API result keys:', result ? Object.keys(result) : 'null');
        ...
        if (!response) {
            ...
            logger.error('Result type:', typeof result);
            logger.error('Result keys:', Object.keys(result));
            if (result.response) {
        ...
        const validClicks = jsonResponse.clicks.filter(click => {
            ...
            if (click.x < 0 || click.x > actualWidth || click.y < 0 || click.y > actualHeight) {
                logger.warn(`⚠️  Skipping out-of-bounds click: (${click.x}, ${click.y}) - Image: ${actualWidth}x${actualHeight}`);
                return false;
            }
            return true;
        });
    ...
    try {
        ...
        const clickPoints = await analyzeBoundingBoxWithGemini(
            ...
            { 
                ...
                displayWidth: canvasInfo.displayWidth,
                displayHeight: canvasInfo.displayHeight
            },
            apiKey,
            enableScreenshot
        );
        ...
        if (frameElement) {
            ...
            logger.debug(`📍 Iframe offset (with scroll): (${iframeOffset.x}, ${iframeOffset.y})`);
        } else {
            logger.warn('⚠️  Could not get iframe element, clicks may be inaccurate');
        ...
        for (let i = 0; i < clickPoints.length; i++) {
            ...
            const canvasPosition = await frame.evaluate((canvasSelector, pointX, pointY) => {
                ...
                return {
                    frameRelativeX: rect.left + relativeX,
                    frameRelativeY: rect.top + relativeY,
            ...
            const pageX = canvasPosition.frameRelativeX + iframeOffset.x;
            const pageY = canvasPosition.frameRelativeY + iframeOffset.y;
            
            if (!isFinite(pageX) || !isFinite(pageY)) {
                logger.error(`  ✗ Invalid position for point ${i + 1}: (${pageX}, ${pageY})`);
                continue;
            }
            
            logger.debug(`  📐 Point ${i + 1}: Canvas(${canvasPosition.original.x}, ${canvasPosition.original.y}) → Frame(${canvasPosition.frameRelativeX.toFixed(1)}, ${canvasPosition.frameRelativeY.toFixed(1)}) → Page(${pageX.toFixed(1)}, ${pageY.toFixed(1)})`);
        ...
        for (const pos of clickPositions) {
            logger.info(`  → Clicking point ${pos.index}/${clickPositions.length} at viewport (${pos.x}, ${pos.y})...`);
            
            await cursor.moveTo(
                { x: pos.x, y: pos.y },
                { 
                    hesitate: Math.random() * 80 + 40,
                    moveDelay: Math.random() * 300 + 150,
            ...
            await page.mouse.click(pos.x, pos.y, { delay: 50 + Math.random() * 50 });
            
            logger.info(`  ✓ Clicked point ${pos.index}/${clickPositions.length} (canvas: ${Math.round(pos.originalX)}, ${Math.round(pos.originalY)})`);
