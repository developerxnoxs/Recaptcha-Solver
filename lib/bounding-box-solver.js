...
async function analyzeBoundingBoxWithGemini(screenshotPath, promptText, canvasSize, apiKey, enableScreenshot = false) {
    ...
    try {
        ...
        const geminiPrompt = `You are analyzing a hCaptcha area selection challenge. This is a canvas-based challenge where you need to click on specific objects.

TASK: ${promptText}

CANVAS INFO:
- Size: ${canvasSize.width}x${canvasSize.height} pixels
- Contains multiple icons/objects displayed on the canvas
- You need to identify and locate the objects that match the task description

INSTRUCTIONS:
1. Carefully examine the entire canvas image
2. Identify ALL objects/icons that match: "${promptText}"
3. For each matching object, determine its CENTER point coordinates
4. Common hCaptcha bounding box tasks include:
   - "Click on the two icons that are different from the others"
   - "Click on the largest object"
   - "Click on all motorcycles"
   - "Click on objects that don't match"
   
5. Object types you might encounter:
   - Icons: simple graphic symbols
   - Animals: cats, dogs, birds, elephants
   - Vehicles: cars, motorcycles, buses, bicycles
   - Common objects: chairs, tables, phones, umbrellas
   - Shapes and symbols

CRITICAL RULES:
- Return coordinates for the CENTER (middle point) of each object
- Coordinates must be within bounds: x (0-${canvasSize.width}), y (0-${canvasSize.height})
- If multiple objects match, return ALL of them
- If NO objects match the criteria, return empty clicks array []
- Be accurate with pixel coordinates - precision matters
...
RESPONSE FORMAT:
{
  ...
  "reasoning": "Identified 2 motorcycles - one in top-left, one in center-right"
}
