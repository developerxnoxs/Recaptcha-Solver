...
async function solveCanvasBasedDrag(page, frame, promptText, apiKey, screenshotDir, enableScreenshot = false, maxAttempts = 2) {
    ...
    try {
        ...
        // Take screenshot of FULL CHALLENGE AREA (including instruction text)
        const timestamp = Date.now();
        const screenshotPath = path.join(screenshotDir, `canvas_drag_${timestamp}.png`);
        
        // Get challenge area that includes instruction text and canvas
        const challengeArea = await frame.$('.challenge-view, .challenge-container, .task');
        if (challengeArea) {
            await challengeArea.screenshot({
                path: screenshotPath,
                type: 'png'
            ...
            // Get canvas position within the screenshot (offset from top)
            const canvasOffsetInChallenge = await frame.evaluate(() => {
                const challenge = document.querySelector('.challenge-view, .challenge-container, .task');
                const canvas = document.querySelector('canvas');
                if (!challenge || !canvas) return { x: 0, y: 0 };
                
                const challengeRect = challenge.getBoundingClientRect();
                const canvasRect = canvas.getBoundingClientRect();
                
                return {
                    x: canvasRect.left - challengeRect.left,
                    y: canvasRect.top - challengeRect.top
                };
            });
            
            // Store offset for coordinate adjustment later
            canvasInfo.offsetInScreenshot = canvasOffsetInChallenge;
            
            if (enableScreenshot) {
                logger.info(`📸 Full challenge screenshot saved (with instruction text): ${screenshotPath}`);
                logger.info(`📏 Canvas offset in screenshot: (${canvasOffsetInChallenge.x}, ${canvasOffsetInChallenge.y})`);
            }
            ...
            // CRITICAL FIX (Based on Web References):
            // Screenshot from canvasElement.screenshot() is taken at DISPLAY SIZE (CSS pixels)
            // So Gemini's coordinates are ALREADY in display pixel space
            // Reference: https://stackoverflow.com/questions/55677/how-do-i-get-the-coordinates-of-a-mouse-click-on-a-canvas-element
            
            logger.info(`🔧 Canvas Coordinate Transform:`);
            logger.info(`   Canvas intrinsic: ${canvasInfo.width}x${canvasInfo.height}`);
            logger.info(`   Canvas display (CSS): ${canvasInfo.displayWidth}x${canvasInfo.displayHeight}`);
            logger.info(`   Screenshot analyzed by Gemini is at DISPLAY size (${canvasInfo.displayWidth}x${canvasInfo.displayHeight})`);
            logger.info(`   Gemini coordinates are in CSS pixel space, NOT canvas intrinsic space`);
            
            // Gemini coordinates are ALREADY in display (CSS) pixel space
            // We only need to:
            // 1. Add canvas position within iframe
            // 2. Add iframe position within page
            
            // Get iframe position within page (using frame.evaluate to run in parent context)
            const frameElement = await frame.frameElement();
            let iframePosition = { x: 0, y: 0 };
            
            if (frameElement) {
                // Run in PARENT window context to get iframe position
                iframePosition = await frameElement.evaluate((iframe) => {
                    const rect = iframe.getBoundingClientRect();
                    return {
                        x: rect.left,
                        y: rect.top
                ...
                logger.debug(`🖼️  Iframe position in parent: (${iframePosition.x.toFixed(1)}, ${iframePosition.y.toFixed(1)})`);
            }
            
            // Get canvas position within iframe (run in iframe context)
            const canvasPositionInFrame = await frame.evaluate(() => {
                const canvas = document.querySelector('canvas');
            ...
            logger.debug(`🎨 Canvas position in iframe: (${canvasPositionInFrame.x.toFixed(1)}, ${canvasPositionInFrame.y.toFixed(1)})`);
            
            // Get zoom level from page
            ...
            if (zoomLevel !== 1) {
                logger.info(`🔍 Page zoom detected: ${zoomLevel}x - Adjusting coordinates`);
            }
            
            // Simple transformation: Gemini coords + canvas position in iframe + iframe position in page
            // Then multiply by zoom level to adjust for browser zoom
            // This matches the proven formula from web references
            let pageSourceX = (analysis.source.x + canvasPositionInFrame.x + iframePosition.x) * zoomLevel;
            let pageSourceY = (analysis.source.y + canvasPositionInFrame.y + iframePosition.y) * zoomLevel;
            let pageTargetX = (analysis.target.x + canvasPositionInFrame.x + iframePosition.x) * zoomLevel;
            let pageTargetY = (analysis.target.y + canvasPositionInFrame.y + iframePosition.y) * zoomLevel;
            
            logger.debug(`📐 Coordinate Calculation:`);
            logger.debug(`   Gemini source: (${analysis.source.x}, ${analysis.source.y})`);
            logger.debug(`   + Canvas in iframe: (${canvasPositionInFrame.x.toFixed(1)}, ${canvasPositionInFrame.y.toFixed(1)})`);
            logger.debug(`   + Iframe in page: (${iframePosition.x.toFixed(1)}, ${iframePosition.y.toFixed(1)})`);
            logger.debug(`   = Final page coords: (${pageSourceX.toFixed(1)}, ${pageSourceY.toFixed(1)})`);
...
async function analyzeCanvasDragWithGemini(screenshotPath, promptText, apiKey, canvasInfo, enableScreenshot = false) {
    ...
    try {
        ...
        const offsetY = canvasInfo.offsetInScreenshot?.y || 0;
...
📐 IMAGE INFO:
- This screenshot shows the FULL CHALLENGE AREA with:
  • Instruction text at the TOP ("${promptText}")
  • Canvas puzzle BELOW the instruction text
- Canvas starts at Y-offset: ${offsetY}px from the top of the screenshot
...
3. **MEASURE PIXEL COORDINATES**:
   - IMPORTANT: The canvas starts at Y=${offsetY}px from the top of this screenshot
   - Measure from the TOP-LEFT corner (0, 0) of the CANVAS ELEMENT
   - So if you see a point at Y=100 on the canvas, the coordinate should be Y=100 (NOT Y=${offsetY + 100})
   - Coordinates should be relative to CANVAS top-left corner, not the screenshot top-left
...
**CRITICAL**: 
- Coordinates are relative to CANVAS (not screenshot)
