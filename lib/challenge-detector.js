...
async function detectChallengeType(frame) {
    try {
        const challengeInfo = await frame.evaluate(() => {
            ...
            const promptElement = document.querySelector('.prompt-text, .challenge-prompt, .task-prompt, .prompt, h2.prompt-text');
            if (promptElement) {
                info.prompt = {
            ...
            const multipleChoiceContainer = document.querySelector('.image_label_multiple_choice, .multiple-choice, .answer-container');
            const choiceButtons = document.querySelectorAll('.answer-button, .choice-button, [data-choice], .multiple-choice-option');
            ...
            if (canvas) {
                const hasMultiplePoints = info.prompt && (
                    info.prompt.text.includes('two icons') ||
                    info.prompt.text.includes('multiple') ||
                    info.prompt.text.includes('all')
                );
