        ...
        try {
            ...
            while (this.isWatching) {
                try {
                    ...
                    const captchaFrame = frames.find(frame => frame.url().includes('hcaptcha.com') && frame.url().includes('frame=checkbox'));
    ...
    async _pollForChallengeFrame() {
        try {
            while (this.isWatching) {
                try {
                    const frames = await this.page.frames();
                    const challengeFrame = frames.find(frame => frame.url().includes('hcaptcha.com') && frame.url().includes('frame=challenge'));
                    ...
                    if (challengeFrame) {
                        ...
                        while (this.isWatching && checkCount < 50) {
                            ...
                            const desc = document.querySelector('.prompt-text, .challenge-prompt, .task-prompt');
