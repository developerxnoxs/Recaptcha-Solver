...
class CaptchaWatcher {
    ...
    async _pollForCaptchaReady() {
        try {
            while (this.isWatching) {
                ...
                const captchaFrame = frames.find(frame => frame.url().includes('hcaptcha.com') && !frame.url().includes('hcaptcha.html'));
    ...
    async _pollForChallengeFrame() {
        try {
            while (this.isWatching) {
                const frames = await this.page.frames();
                const challengeFrame = frames.find(frame => frame.url().includes('hcaptcha.com') && frame.url().includes('hcaptcha.html'));
