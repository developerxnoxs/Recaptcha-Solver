...
async function solveJigsawChallenge(page, frame, promptText, apiKey, screenshotDir, enableScreenshot = false, maxAttempts = 2) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
            ...
            logger.info(`  🔄 Dragging cursor in ${steps} smooth steps...`);
            
            for (let i = 1; i <= steps; i++) {
                const currentX = startX + (deltaX * i);
                const currentY = startY + (deltaY * i);
                
                const jitter = i % 3 === 0 ? (Math.random() - 0.5) * 2 : 0;
                
                await cursor.moveTo(
                    { x: currentX + jitter, y: currentY },
                    {
                        hesitate: 0,
                ...
                if (i % 5 === 0) {
                    logger.debug(`    Progress: ${Math.round((i / steps) * 100)}% (${i}/${steps})`);
                }
            }
