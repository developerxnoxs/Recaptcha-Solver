...
async function analyzeWithGemini(screenshotPath, prompt, gridType, apiKey, enableScreenshot = false) {
    ...
    try {
        ...
        const finalPrompt = `You are analyzing a hCaptcha challenge image grid. Your task is to identify which tiles contain the specified object.
...
INSTRUCTIONS:
1. Examine each tile carefully in the grid image
2. Look for CLEAR, VISIBLE instances of "${mainPrompt}"
3. The object can be:
   - Partially visible (object cut off by tile edges) - STILL COUNTS as a match
   - At any angle or orientation
   - Any size within the tile
   - In various contexts (indoor, outdoor, different backgrounds)
   
4. Common object categories to be aware of:
   - Vehicles: cars, buses, motorcycles, bicycles, tractors, airplanes, boats
   - Animals: cats, dogs, horses, elephants, birds, fish
   - Objects: bridges, traffic lights, fire hydrants, stairs, chimneys
   - Nature: trees, mountains, water, flowers
   - Structures: buildings, houses, windows, doors
   - Everyday items: phones, chairs, tables, umbrellas

CRITICAL RULES:
- ONLY mark a tile as "has_match": true if you are CONFIDENT the object is present
- If "${mainPrompt}" does NOT appear in ANY tile, mark ALL tiles as "has_match": false
- PARTIAL objects count - if you can clearly identify it as "${mainPrompt}" even when cut off
- When UNCERTAIN or the object is AMBIGUOUS, mark as "has_match": false
- FALSE POSITIVES are worse than missing tiles - be conservative and precise
- Better to miss an uncertain tile than to incorrectly mark a wrong tile

RESPONSE FORMAT:
Respond ONLY with a JSON object where each key is the tile coordinate [row,col] and the value is an object with "has_match" boolean.

Example: {"[1,1]": {"has_match": false}, "[1,2]": {"has_match": true}, ...}
...
async function solveChallengeLoop(watcher, apiKey, screenshotDir, enableScreenshot = false) {
    ...
    if (challengeType.type === 'BOUNDING_BOX') {
        ...
        logger.warn('❌ Bounding box challenge failed');
        return null;
    }
    ...
    if (challengeType.type === 'JIGSAW_SLIDER') {
        ...
        logger.warn('❌ Jigsaw challenge failed');
        return null;
    }
