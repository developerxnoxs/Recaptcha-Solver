...
async function getPageZoomLevel(page) {
    try {
        const zoomLevel = await page.evaluate(() => {
            // Get the actual zoom level from browser
            return window.devicePixelRatio / (window.outerWidth / window.innerWidth);
        });
        return zoomLevel || 1;
    } catch (error) {
        logger.debug(`Failed to get zoom level: ${error.message}`);
        return 1;
    }
}
...
async function analyzeWithGemini(screenshotPath, prompt, gridType, apiKey, enableScreenshot = false) {
    ...
    try {
        ...
        const finalPrompt = `You are an EXPERT visual recognition AI analyzing a hCaptcha challenge. Your accuracy MUST be 100% - this challenge MUST be solved in ONE attempt.

🎯 TARGET OBJECT TO FIND: **${mainPrompt.toUpperCase()}**
...
1. Study the target object: "${mainPrompt}"
2. Identify its KEY visual characteristics (shape, color, texture, unique features)
3. Consider what makes it DIFFERENT from similar objects
4. Remember: You are looking for THE EXACT OBJECT, not similar items

**PHASE 2: ANALYZE EACH TILE SYSTEMATICALLY**
For EACH tile, ask yourself these questions:
   a) "Can I see the target object '${mainPrompt}' in this tile?"
   b) "Is it CLEARLY visible (not blurry or ambiguous)?"
   c) "Am I 100% CERTAIN this is '${mainPrompt}' and not something similar?"
   d) "Would a human with perfect vision mark this tile?"

**PHASE 3: STRICT INCLUSION RULES**
✅ MARK AS TRUE only if:
   - The object is CLEARLY identifiable as "${mainPrompt}"
   - You are 95%+ confident (not 85%, not 90% - minimum 95%)
   - The object is the PRIMARY subject of the tile (not background)
   - Even if partially visible (>50% visible), you can definitively identify it
   - The object matches ALL key characteristics of "${mainPrompt}"

❌ MARK AS FALSE if:
   - ANY uncertainty or doubt exists (even slight hesitation → FALSE)
   - Object is blurry, dark, or obscured
   - Object is a SIMILAR but DIFFERENT item (bike ≠ motorcycle, car ≠ truck)
   - Object is a reflection, shadow, drawing, or image-within-image
   - Object is too small/distant to identify with certainty
   - Less than 50% of the object is visible

**PHASE 4: COMMON CONFUSIONS TO AVOID**
🚗 VEHICLES:
   - "car" ≠ truck, SUV, van, pickup (be strict with vehicle types)
   - "motorcycle" ≠ bicycle, scooter, moped
   - "bus" ≠ van, large truck, RV
...
🏗️ STRUCTURES:
   - "bridge" ≠ overpass, flyover, viaduct (unless they're synonyms for the target)
   - "stairs" ≠ ladder, escalator, ramp
   - "chimney" ≠ tower, pole, antenna
   
🚦 STREET OBJECTS:
   - "traffic light" ≠ street light, lamp post
   - "fire hydrant" ≠ mailbox, post, bollard
   
🌳 NATURE:
   - "tree" ≠ bush, shrub, plant (unless clearly a tree)
   - "mountain" ≠ hill, rock formation
   - "flower" ≠ general plant, weed

**PHASE 5: DOUBLE-CHECK YOUR WORK**
Before finalizing:
   1. Count how many tiles you marked as true
   2. Re-examine EACH true marking one more time
   3. Ask: "If I'm wrong on any of these, the challenge FAILS. Am I ABSOLUTELY certain?"
   4. If you have ANY doubt on a tile, change it to false
   5. It's better to miss 1-2 tiles (and do a second round) than to mark wrong tiles
...
📊 REQUIRED JSON RESPONSE FORMAT:
{
  ...
  "[1,2]": {"has_match": true},
  "[1,3]": {"has_match": false},
  ... (continue for all ${gridType.includes('44') ? '16' : '9'} tiles)
}

🎯 FINAL REMINDER: You are looking for "${mainPrompt}" - be PRECISE, be CERTAIN, be ACCURATE.
⚠️ Respond with ONLY valid JSON. No explanations, no markdown, no text - JUST JSON.`;
...
async function clickTile(frame, coord, challengeInfo) {
    try {
        ...
        let pageX = (tilePosition.frameRelativeX + iframeOffset.x) * zoomLevel;
        let pageY = (tilePosition.frameRelativeY + iframeOffset.y) * zoomLevel;
        
        if (zoomLevel !== 1) {
            logger.debug(`🔍 Zoom detected: ${zoomLevel}x - Adjusting coordinates`);
        }
