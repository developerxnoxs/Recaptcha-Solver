...
async function solveCanvasBasedDrag(page, frame, promptText, apiKey, screenshotDir, enableScreenshot = false, maxAttempts = 2) {
    ...
    try {
        ...
        if (canvasElement) {
            ...
            // CRITICAL FIX: Screenshot is taken at DISPLAY size, not canvas intrinsic size!
            // Gemini analyzes the screenshot which is already at display resolution (500x430)
            // So coordinates from Gemini are ALREADY in display space - NO SCALING NEEDED!
            logger.info(`🔧 Canvas Coordinate Analysis:`);
            ...
            logger.info(`   Screenshot analyzed by AI is at DISPLAY size`);
            logger.info(`   AI coordinates are ALREADY in display space - using directly`);
            
            // Use coordinates directly from Gemini (they're already in display space)
            const displaySourceX = analysis.source.x;
            const displaySourceY = analysis.source.y;
            const displayTargetX = analysis.target.x;
            const displayTargetY = analysis.target.y;
