...
/**
 * Helper function to normalize canvas coordinates
 * Scales coordinates from canvas intrinsic size to display size
 * @param {Object} point - Point with x,y coordinates (from AI analysis on full-size screenshot)
 * @param {Object} canvasInfo - Canvas information with width, height, displayWidth, displayHeight
 * @returns {Object} - Scaled point {x, y} for display coordinates
 */
function normalizeCanvasPoint(point, canvasInfo) {
    if (!canvasInfo || !canvasInfo.width || !canvasInfo.displayWidth) {
        logger.warn('⚠️ Missing canvas info for normalization, returning original point');
        return point;
    }
    ...
    logger.debug(`🔧 Normalized point: (${point.x}, ${point.y}) × (${scaleX.toFixed(3)}, ${scaleY.toFixed(3)}) = (${normalized.x.toFixed(1)}, ${normalized.y.toFixed(1)})`);
    
    return normalized;
}

/**
 * Get challenge area dimensions for scaling calculations
 * Returns both screenshot size and display size for proper coordinate scaling
 */
async function getChallengeAreaInfo(frame) {
    return await frame.evaluate(() => {
        const challengeArea = document.querySelector('.challenge-view, .challenge-container');
        if (!challengeArea) return null;
        
        const rect = challengeArea.getBoundingClientRect();
        const canvas = challengeArea.querySelector('canvas');
...
async function solveCanvasBasedDrag(page, frame, promptText, apiKey, screenshotDir, enableScreenshot = false, maxAttempts = 2) {
    ...
    try {
        ...
        // Take screenshot of CANVAS ONLY (not container) to avoid coordinate frame mismatch
        const timestamp = Date.now();
        ...
        if (canvasElement) {
            ...
            if (enableScreenshot) {
                logger.info(`📸 Canvas screenshot saved (canvas element only): ${screenshotPath}`);
            }
            ...
            // Get iframe offset and visual viewport scale for coordinate translation
            const frameElement = await frame.frameElement();
            let iframeOffset = { x: 0, y: 0 };
            ...
            if (frameElement) {
                transformInfo = await frameElement.evaluate((iframe) => {
                    const rect = iframe.getBoundingClientRect();
                    const scrollX = window.scrollX || window.pageXOffset || 0;
                    const scrollY = window.scrollY || window.pageYOffset || 0;
                    
                    // CRITICAL: Get visual viewport scale for mobile/responsive views
                    const vv = window.visualViewport || { scale: 1, offsetLeft: 0, offsetTop: 0 };
                    
                    // Get computed transform matrix from CSS if exists (with cross-origin safety)
                    let cssScale = { x: 1, y: 1 };
                    ...
                    try {
                        ...
                        // Cross-origin iframe - can't access contentDocument, use defaults
                    }
                ...
                iframeOffset = transformInfo.iframeOffset;
                viewportScale = transformInfo.viewportScale;
                viewportOffset = transformInfo.viewportOffset;
                
                logger.debug(`🖼️  Iframe offset (with scroll): (${iframeOffset.x.toFixed(1)}, ${iframeOffset.y.toFixed(1)})`);
                logger.debug(`📱 Visual viewport scale: (${viewportScale.x.toFixed(3)}, ${viewportScale.y.toFixed(3)})`);
                logger.debug(`📍 Visual viewport offset: (${viewportOffset.x.toFixed(1)}, ${viewportOffset.y.toFixed(1)})`);
                logger.debug(`🎨 CSS scale: (${transformInfo.cssScale.x.toFixed(3)}, ${transformInfo.cssScale.y.toFixed(3)})`);
            }
            
            // COORDINATE TRANSFORMATION CHAIN (Mobile/Responsive-Aware):
            // Problem: Gemini analyzes unscaled canvas screenshot, but getBoundingClientRect()
            //          returns viewport-scaled positions. We must scale Gemini coords to match.
            // 
            // Transform pipeline per axis:
            // 1. Scale Gemini coords by visualViewport.scale and CSS transforms
            // 2. Add canvas position within iframe (already viewport-scaled from getBoundingClientRect)
            // 3. Add iframe offset within top page
            // 4. Add visualViewport offset
            
            logger.info(`🔧 Canvas Coordinate Transform (Mobile/Responsive-Aware):`);
            logger.info(`   Canvas intrinsic: ${canvasInfo.width}x${canvasInfo.height}`);
            logger.info(`   Canvas display: ${canvasInfo.displayWidth}x${canvasInfo.displayHeight}`);
            logger.info(`   Visual viewport scale: ${viewportScale.x.toFixed(3)}x`);
            logger.info(`   CSS transform scale: ${(transformInfo?.cssScale?.x || 1).toFixed(3)}x`);
            
            // Get CSS scale from transformInfo
            const cssScale = transformInfo?.cssScale || { x: 1, y: 1 };
            
            // Step 1: Scale Gemini coords by viewport and CSS transforms
            // Gemini coords are from unscaled screenshot, but canvas position is viewport-scaled
            const sourceX_viewport = analysis.source.x * viewportScale.x * cssScale.x;
            const sourceY_viewport = analysis.source.y * viewportScale.y * cssScale.y;
            const targetX_viewport = analysis.target.x * viewportScale.x * cssScale.x;
            const targetY_viewport = analysis.target.y * viewportScale.y * cssScale.y;
            
            logger.debug(`📐 Transform step 1 - Apply viewport & CSS scale:`);
            logger.debug(`   Source X: ${analysis.source.x} * ${viewportScale.x.toFixed(3)} * ${cssScale.x.toFixed(3)} = ${sourceX_viewport.toFixed(1)}`);
            logger.debug(`   Source Y: ${analysis.source.y} * ${viewportScale.y.toFixed(3)} * ${cssScale.y.toFixed(3)} = ${sourceY_viewport.toFixed(1)}`);
            logger.debug(`   Target X: ${analysis.target.x} * ${viewportScale.x.toFixed(3)} * ${cssScale.x.toFixed(3)} = ${targetX_viewport.toFixed(1)}`);
            logger.debug(`   Target Y: ${analysis.target.y} * ${viewportScale.y.toFixed(3)} * ${cssScale.y.toFixed(3)} = ${targetY_viewport.toFixed(1)}`);
            
            // Step 2: Add canvas position within iframe (from getBoundingClientRect, already scaled)
            const sourceX_iframe = sourceX_viewport + canvasInfo.x;
            const sourceY_iframe = sourceY_viewport + canvasInfo.y;
            const targetX_iframe = targetX_viewport + canvasInfo.x;
            const targetY_iframe = targetY_viewport + canvasInfo.y;
            
            logger.debug(`📐 Transform step 2 - Add canvas position in iframe:`);
            logger.debug(`   Source: (${sourceX_viewport.toFixed(1)}, ${sourceY_viewport.toFixed(1)}) + (${canvasInfo.x.toFixed(1)}, ${canvasInfo.y.toFixed(1)}) = (${sourceX_iframe.toFixed(1)}, ${sourceY_iframe.toFixed(1)})`);
            
            // Step 3: Add iframe offset within top page
            const sourceX_top = sourceX_iframe + iframeOffset.x;
            const sourceY_top = sourceY_iframe + iframeOffset.y;
            const targetX_top = targetX_iframe + iframeOffset.x;
            const targetY_top = targetY_iframe + iframeOffset.y;
            
            logger.debug(`📐 Transform step 3 - Add iframe offset in page:`);
            logger.debug(`   Source: (${sourceX_iframe.toFixed(1)}, ${sourceY_iframe.toFixed(1)}) + (${iframeOffset.x.toFixed(1)}, ${iframeOffset.y.toFixed(1)}) = (${sourceX_top.toFixed(1)}, ${sourceY_top.toFixed(1)})`);
            
            // Step 4: Add visualViewport offset (for pinch-zoom scenarios)
            const pageSourceX = sourceX_top + viewportOffset.x;
            const pageSourceY = sourceY_top + viewportOffset.y;
            const pageTargetX = targetX_top + viewportOffset.x;
            const pageTargetY = targetY_top + viewportOffset.y;
            
            logger.debug(`📐 Transform step 4 - Add viewport offset:`);
            logger.debug(`   Source: (${sourceX_top.toFixed(1)}, ${sourceY_top.toFixed(1)}) + (${viewportOffset.x.toFixed(1)}, ${viewportOffset.y.toFixed(1)}) = (${pageSourceX.toFixed(1)}, ${pageSourceY.toFixed(1)})`);
...
📐 IMAGE INFO:
- This screenshot shows ONLY the canvas element (no container padding/borders)
- Canvas intrinsic size: ${canvasInfo.width}x${canvasInfo.height} pixels
- Canvas display size: ${canvasInfo.displayWidth}x${canvasInfo.displayHeight} pixels
- Screenshot is captured at DISPLAY size (${canvasInfo.displayWidth}x${canvasInfo.displayHeight})
...
3. **MEASURE PIXEL COORDINATES**:
   - Measure from the TOP-LEFT corner (0, 0) of the canvas (which is also the screenshot's top-left)
   - Give EXACT pixel coordinates for both source and target centers
   - Coordinates are relative to the canvas element shown in this screenshot
...
async function solveJigsawChallenge(page, frame, promptText, apiKey, screenshotDir, enableScreenshot = false, maxAttempts = 2) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
            ...
            // CRITICAL: Get challenge area dimensions for scaling calculations
            const challengeAreaInfo = await getChallengeAreaInfo(frame);
            if (!challengeAreaInfo) {
                logger.error('Could not get challenge area info');
                if (attempt === MAX_ATTEMPTS) return false;
                continue;
            }
            
            // Calculate scaling factors between screenshot and display
            const scaleX = challengeAreaInfo.displayWidth / challengeAreaInfo.width;
            const scaleY = challengeAreaInfo.displayHeight / challengeAreaInfo.height;
            
            logger.info(`📐 Challenge Area Dimensions:`);
            logger.info(`   • Screenshot size: ${challengeAreaInfo.width}×${challengeAreaInfo.height}px`);
            logger.info(`   • Display size: ${challengeAreaInfo.displayWidth}×${challengeAreaInfo.displayHeight}px`);
            logger.info(`   • Scale factors: X=${scaleX.toFixed(4)}, Y=${scaleY.toFixed(4)}`);
            
            if (Math.abs(scaleX - 1.0) > 0.01 || Math.abs(scaleY - 1.0) > 0.01) {
                logger.warn(`⚠️ SCALING DETECTED! Screenshot size differs from display size.`);
                logger.warn(`   This means coordinates from AI need to be scaled for accurate drag.`);
            }
            ...
            // CRITICAL FIX: Scale offset from Gemini analysis if screenshot size != display size
            // Gemini analyzes the SCREENSHOT at full resolution, but we need to apply offsets at DISPLAY resolution
            let offsetX = analysis.horizontal_offset * scaleX;
            let offsetY = analysis.vertical_offset * scaleY;
            
            logger.info(`📏 Offset Scaling:`);
            logger.info(`   • Original offset from AI: (${analysis.horizontal_offset}, ${analysis.vertical_offset})px`);
            logger.info(`   • Scaled offset for display: (${Math.round(offsetX)}, ${Math.round(offsetY)})px`);
